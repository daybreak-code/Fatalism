...
public class ClientDefaultAccessTokenConverter implements AccessTokenConverter {
    ...
    public ClientDefaultAccessTokenConverter(UserAuthenticationConverter userAuthenticationConverter) {
    ...
    public Map<String, ?> convertAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        ...
        if (!authentication.isClientOnly()){
            ...
            if (authentication.getOAuth2Request().getAuthorities() != null && !authentication.getOAuth2Request().getAuthorities().isEmpty()){
                response.put(CLIENT_AUTHORITIES, AuthorityUtils.authorityListToSet(authentication.getOAuth2Request().getAuthorities()));
            }
        } else {
            if (clientToken.getAuthorities() != null && !clientToken.getAuthorities().isEmpty()){
                response.put(
                        UserAuthenticationConverter.AUTHORITIES,
        ...
        if (token.getExpiration() != null){
            response.put(EXP, token.getExpiration());
        }
